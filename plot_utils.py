...
def residuals(params, data_rate, y, bandwidth, metadata):
    snr_eff, eta = params
    pred = shannon_fit(data_rate, snr_eff, eta, bandwidth, metadata)
...
def shannon_fit(data_rate, snr_eff, eta,bandwidth, metadata):
...
def graph_sensitivity_phyrate(metadata: dict,
    ...
    popt, pcov = curve_fit(
        lambda dr, snr_eff, eta:
            shannon_fit(dr, snr_eff, eta, closest_bandwidth, metadata=metadata),
        data_rates,
        sensitivities,
        p0=[0.1, 0.8]
    )
    ...
    residuals,
    x0=[0.1, 0.8],
    args=(data_rates, sensitivities,closest_bandwidth, metadata)
    )
