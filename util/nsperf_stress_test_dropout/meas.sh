...
num_conns_values="${NUM_CONNS:-1 2 4 6 8 10 15}"
failure_probabilities="${FAILURE_PROBABILITIES:-0 0.01 0.05}"
replacement_delays="${REPLACEMENT_DELAYS:-30}"
dropout_time_steps="${DROPOUT_TIME_STEPS:-10}"
						...
						for loss in $losses; do
							...
							while [ "$i" -le "$iterations" ]; do
								...
								msh_progress_start_context "$script_start_epoch" "$completed_simulations" \
									"$simulation_idx" "$total_simulations" "$progress_context"
